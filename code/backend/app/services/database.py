...
def get_collection(force_refresh: bool = False):
    ...
    if _submissions_collection is None or force_refresh:
        ...
        print(
            f"📊 Collection loaded: {_submissions_collection.count()} total submissions"
        )
...
def add_submissions(
    submissions: List[str], project_id: str, user_ids: Optional[List[str]] = None
) -> List[str]:
    """
    ...
    collection.add(documents=submissions, ids=submission_ids, metadatas=metadatas)
...
def search_similar_submissions(
    query: str, project_id: str, n_results: int = 10
) -> Dict[str, Any]:
    """
