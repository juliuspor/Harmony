...
Summary:"""
    ...
    try:
        ...
        logger.info(
            f"Summarizing cluster {cluster_index + 1} with model {config.OPENAI_MODEL}"
        )
        ...
        logger.info(
            f"Generated summary for cluster {cluster_index + 1} (length: {len(summary)})"
        )
        return summary
        ...
        logger.error(
            f"Summarization error for cluster {cluster_index + 1}: {error_msg}"
        )
        return error_msg
...
async def summarize_cluster(texts: List[str], cluster_index: int) -> str:
    ...
    return await loop.run_in_executor(
        _executor, _summarize_cluster_sync, texts, cluster_index
    )
...
Two-word title:"""
    ...
    try:
        ...
        logger.error(
            f"Title generation error for cluster {cluster_index + 1}: {str(e)}"
        )
        return "Untitled"
...
async def generate_title(texts: List[str], cluster_index: int) -> str:
    ...
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(
        _executor, _generate_title_sync, texts, cluster_index
    )
