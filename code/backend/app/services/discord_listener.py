...
def add_submission(
    ...
    try:
        ...
        print(
            f"✅ New Discord submission saved for project {project_id} from user {user_id}: {message[:50]}..."
        )
    ...
    async def on_message(self, message: discord.Message):
        ...
        try:
            ...
            if message_id in _processed_messages:
                print(
                    f"⏭️ Already processed Discord message {message_id}, skipping duplicate"
                )
                return

            # Log ALL incoming messages for debugging
            print(
                f"📬 Incoming Discord message: channel={channel_id}, user={username}, bot={message.author.bot}, text='{text[:50]}...'"
            )
            ...
            if channel_id in _active_monitors:
                ...
                print(
                    f"✅ Valid user message for monitored Discord channel {channel_id}, saving to project {project_id}"
                )
                ...
                print(
                    f"ℹ️ Message in unmonitored Discord channel {channel_id}, skipping. Monitored channels: {list(_active_monitors.keys())}"
                )
    ...
    if _discord_client:
        try:
            ...
            asyncio.run_coroutine_threadsafe(
                _discord_client.close(), _discord_client.loop
            )
...
def post_discord_message_to_channel(message: str, channel_id: str) -> Dict[str, Any]:
    ...
    if not _discord_client or not _discord_client.is_ready():
        raise ValueError(
            "Discord bot is not connected. Please check DISCORD_BOT_TOKEN and ensure the bot is running."
        )
    ...
    async def _send_message():
        try:
            ...
            if not channel:
                raise ValueError(
                    f"Discord channel {channel_id} not found or bot doesn't have access"
                )
